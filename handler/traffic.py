...
from constants.header import HeaderDataFrameConstant
...
class TrafficHandler:
    ...
    def __format_traffic(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            df = data.copy()
            df.drop(columns=[HeaderDataFrameConstant.ID, HeaderDataFrameConstant.TYPE_LAYER], inplace=True)
            df = df.reindex([
                HeaderDataFrameConstant.NAME,
                HeaderDataFrameConstant.TYPE, 
                HeaderDataFrameConstant.CAPACITY,
                HeaderDataFrameConstant.DATE, 
                HeaderDataFrameConstant.TIME,
                HeaderDataFrameConstant.IN_PROM, 
                HeaderDataFrameConstant.OUT_PROM, 
                HeaderDataFrameConstant.IN_MAX, 
                HeaderDataFrameConstant.OUT_MAX
            ], axis="columns")
            df.rename(columns={
                HeaderDataFrameConstant.NAME: HeaderDataFrameConstant.INTERFACE
            }, inplace=True)
            return df
        except Exception as e:
    ...
    def __format_borde_traffic(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            df = data.copy()
            df.rename(columns={HeaderDataFrameConstant.MODEL: HeaderDataFrameConstant.TYPE}, inplace=True)
    ...
    def __insert_name_layer(self, df: pd.DataFrame, layer_type: str) -> pd.DataFrame:
        ...
        try:
            ...
            if not df_traffic.empty and layer_type == LayerType.BORDE:
                ...
                df_traffic.rename(columns={HeaderDataFrameConstant.ID_LAYER: HeaderDataFrameConstant.ID}, inplace=True)
                df_traffic = df_traffic.merge(df_borde, how="inner", on=HeaderDataFrameConstant.ID)
                df_traffic = self.__format_borde_traffic(data=df_traffic)
