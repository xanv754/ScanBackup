...
class BrasHandler:
    ...
    def __init__(self, db_backup: bool = False):
        try:
            ...
            log = LogHandler()
            log.export(f"Bras handler. Failed connecting to the database. {e}", path=__file__, err=True)
    ...
    def get_all_interfaces(self) -> pd.DataFrame:
        ...
        try:
            ...
            log = LogHandler()
            log.export(f"Bras handler. Failed to get all interfaces of bras layer. {e}", path=__file__, err=True)
            return pd.DataFrame()
        else:
            return df