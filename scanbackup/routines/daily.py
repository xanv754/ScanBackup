...
class GenerateDailySummary:
    ...
    def __init__(
        self,
        date: str = (datetime.now(timezone.utc) - timedelta(days=1)).strftime(
            "%Y-%m-%d"
        ),
        layer: str | None = None,
    ):
        if layer:
            ...
            self.layers = foldername_BBIP_SCAN
        self.date = date
    ...
    def _calculate_summary_data(
        self, interface: str, type: str, capacity: float, data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if in_max_average >= out_max_average:
                use = (in_max_average / capacity) * 100
            else:
                use = (out_max_average / capacity) * 100
            ...
            log.error(
                f"Rutina resúmenes diarios. Fallo en cálculo de la data de BBIP de la interfaz: {interface} - {error}"
            )
            return pd.DataFrame(columns=header_daily_bbip)

    def _calculate_summary_ip_data(
        self, interface: str, type: str, capacity: float, data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            log.error(
                f"Rutina resúmenes diarios. Fallo en cálculo de la data de IPBras de la interfaz: {interface} - {error}"
            )
    ...
    def run(self) -> None:
        ...
        try:
            for layer in self.layers:
                ...
                if layer != LayerName.IP_BRAS:
                    summary_report = pd.DataFrame(columns=header_daily_bbip)
                else:
                    summary_report = pd.DataFrame(columns=header_daily_ip_bras)
                for filename in os.listdir(folder_data):
                    ...
                    if layer != LayerName.IP_BRAS:
                        data = pd.read_csv(
                            filepath, sep=self._separator_data, names=header_scan_bbip
                        )
                ...
                if not summary_report.empty:
                    summary_report.to_csv(
                        self._get_daily_path(layer),
                        index=False,
                        sep=self._separator_data,
                        decimal=self._export_decimal,
                    )
        except Exception as error:
            log.error(
                f"Rutina resúmenes diarios. Generación de reportes diarios fallido. {error}"
            )
        else:
            log.info(
                "Rutina resúmenes diarios. Generación de reportes diarios finalizada"
            )
...
@click.option(
    ...
    type=str,
    required=False,
    help="Especifica la fecha para obtención de los datos para el resumen diario. \t Formato: YYYY-MM-DD. \t Por defecto, si no se especifica, se genera los reportes del día anterior al actual",
)
@click.option(
    "-l",
    "--layer",
    type=click.Choice(foldername_BBIP_SCAN),
    required=False,
    help="Especifica la capa en la que se desea generar el resumen diario. \t Por defecto, si no se especifica, genera el resumen de todas las capas",
)
def generate_daily_summary(layer: str | None = None, date: str | None = None) -> None:
    if date:
        report = GenerateDailySummary(date=date, layer=layer)
    else:
        report = GenerateDailySummary(layer=layer)
    report.run()
