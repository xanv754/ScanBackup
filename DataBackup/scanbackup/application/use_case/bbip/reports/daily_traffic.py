...
class TrafficDailyReportGeneratorUseCase:
    """Generates the daily traffic report of every active interface, grouped by layer, into a single .xlsx file."""
    ...
    def execute(self) -> str:
        """Generate the daily traffic report of every active interface, grouped by layer.

        Reads every active source (regardless of layer) and the stored daily
        summary for the target date, joins them by device, and exports one
        Excel sheet per configured layer, created even when a layer has no
        data for the date.
        ...
        try:
            ...
            raise ExportError(
                message="Error al generar el reporte diario de tráfico",
                error=error,
            )
