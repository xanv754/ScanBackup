...
from scanbackup.domain import IPActiveBBIPEntity
...
MODULE = "scanbackup.infrastructure.persistence.mongodb.repositories.bbip.ip.history"


class TestMongoIPHistoryBBIPRepository(unittest.TestCase):
    """Unit tests for the MongoIPHistoryBBIPRepository."""

    def setUp(self) -> None:
        """Build a sample entity and repository instance for reuse across tests."""
        self.device_id = ObjectId()
        self.entity = IPActiveBBIPEntity(
            date=date(2026, 1, 1),
            time=time(10, 0),
            in_prom=1.0,
            in_max=2.0,
        ...
        self.repository = MongoIPHistoryBBIPRepository("brasip")
    ...
    def test_name_collection_is_built_from_layer_and_suffix(self) -> None:
        ...
        self.assertEqual(self.repository.name_collection, "BRASIP_IP_HISTORY_BBIP")
    ...
    def test_get_by_date_maps_documents_into_entities(
        ...
        entity = result[0]
        self.assertIsInstance(entity, IPActiveBBIPEntity)
        self.assertEqual(entity.date, date(2026, 1, 1))
        self.assertEqual(entity.in_prom, 1.0)
        self.assertEqual(entity.in_max, 2.0)
