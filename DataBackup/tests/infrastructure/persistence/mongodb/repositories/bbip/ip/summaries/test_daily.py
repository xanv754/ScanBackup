...
from scanbackup.domain import IPDailySummaryBBIPEntity
from scanbackup.shared import MongoInsertFailedError, MongoConnectionError

MODULE = (
    "scanbackup.infrastructure.persistence.mongodb.repositories.bbip.ip.summaries.daily"
)


class TestMongoIPDailySummaryBBIPRepository(unittest.TestCase):
    """Unit tests for the MongoIPDailySummaryBBIPRepository."""

    def setUp(self) -> None:
        """Build a repository instance and a sample entity for reuse across tests."""
        self.repository = MongoIPDailySummaryBBIPRepository()
        self.entity = IPDailySummaryBBIPEntity(
            date=date(2026, 1, 1),
            in_prom=1.0,
            in_max=2.0,
            device=ObjectId(),
