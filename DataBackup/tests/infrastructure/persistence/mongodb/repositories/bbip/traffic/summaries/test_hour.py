...
from scanbackup.domain import TrafficHourSummaryBBIPEntity
from scanbackup.shared import MongoInsertFailedError, MongoConnectionError

MODULE = (
    "scanbackup.infrastructure.persistence.mongodb.repositories.bbip.traffic.summaries.hour"
)


class TestMongoTrafficHourSummaryBBIPRepository(unittest.TestCase):
    """Unit tests for the MongoTrafficHourSummaryBBIPRepository."""

    def setUp(self) -> None:
        """Build a repository instance and a sample entity for reuse across tests."""
        self.repository = MongoTrafficHourSummaryBBIPRepository()
        self.entity = TrafficHourSummaryBBIPEntity(
            date=date(2026, 1, 1),
            time=time(13, 0, 0),
            in_prom=1.0,
            in_max=2.0,
