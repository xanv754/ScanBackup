...
class TestMongoTrafficHourSummaryBBIPDTO(unittest.TestCase):
    """Unit tests for the MongoTrafficHourSummaryBBIPDTO from_mongo mapping."""
    ...
    def test_from_mongo_maps_id_to_string(self) -> None:
        ...
        dto = MongoTrafficHourSummaryBBIPDTO.from_mongo(doc)
    ...
    def test_id_defaults_to_none_when_omitted(self) -> None:
        """Building the DTO without an id (e.g. a projected-out '_id') must not fail."""
        dto = MongoTrafficHourSummaryBBIPDTO(
            date=date(2026, 1, 1),
            time=time(13, 0, 0),
            in_prom=1.0,
            out_prom=1.5,
