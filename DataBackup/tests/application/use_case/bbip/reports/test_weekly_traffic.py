...
class TestTrafficWeeklyReportGeneratorUseCaseResolveWeekRange(unittest.TestCase):
    """Unit tests for TrafficWeeklyReportGeneratorUseCase.resolve_week_range."""

    def test_default_mode_spans_the_monday_of_the_previous_week_through_the_current_weeks_sunday(
        self,
    ) -> None:
        """Default (non-literal) mode must span from the previous week's Monday through the current week's Sunday."""
        reference_date = date(2024, 1, 3)  # Wednesday

        start, end = TrafficWeeklyReportGeneratorUseCase.resolve_week_range(
            reference_date, literal=False
        )

        self.assertEqual(start, date(2023, 12, 25))  # Monday
        self.assertEqual(end, date(2023, 12, 31))  # Sunday

    def test_default_mode_when_reference_date_is_itself_a_sunday(self) -> None:
        """When the reference date is a Sunday, it must be treated as the current week's Sunday."""
        reference_date = date(2023, 12, 31)  # Sunday
    ...
    def test_literal_mode_spans_the_7_trailing_days_including_the_reference_date(
        self,
    ) -> None:
        """Literal mode must span the 7 trailing days counting back from the reference date, inclusive."""
        ...
        self.assertEqual(start, date(2023, 12, 28))
        self.assertEqual(end, date(2024, 1, 3))


class TestTrafficWeeklyReportGeneratorUseCase(unittest.TestCase):
    ...
    def test_queries_the_resolved_week_date_range(self) -> None:
    ...
    def test_literal_mode_queries_the_7_trailing_days(self) -> None:
        """execute() in literal mode must query the 7 trailing days from the reference date."""
        self.daily_repo.get_by_date_range.return_value = []
        self.source_repo.get_all_active_sources.return_value = []
