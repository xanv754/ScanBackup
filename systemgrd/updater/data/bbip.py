...
class BBIPUpdaterHandler:
    ...
    def _get_data_link(self, path: str, data_uploading: pd.DataFrame, date: str, force: bool) -> pd.DataFrame:
        ...
        if not df_data.empty:
            ...
            if data_uploading.empty: data_uploading = df_data
            else: data_uploading = pd.concat([data_uploading, df_data], axis=0)
    ...
    def get_data(self, layer: str, date: str | None = None, force: bool = False) -> pd.DataFrame:
    ...
    def load_data(self, layer: str, data: pd.DataFrame, uri: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                log.error(f"Failed to validate data with the model. {layer} updater system has suspended. {e}")
                return False
            else:
                table_name = LayerDetector.get_table_name(layer)
                response = query.new_interface(table_name, json)
                if response: self._clean_data(layer=layer)
                return response
        except Exception as e:
