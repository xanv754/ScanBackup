...
class SourceScrapping(ABC):
    ...
    def _compare_sources_old(
        self, new: pd.DataFrame, old: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            common = pd.merge(
                old, new, on=[HeaderSource.NAME, HeaderSource.MODEL], how="inner"
            )
            common.drop(
            ...
            news.drop(
                columns=[f"{HeaderSource.LINK}_y", f"{HeaderSource.CAPACITY}_y"],
                inplace=True,
            )
            news.rename(
                columns={
                    f"{HeaderSource.LINK}_x": HeaderSource.LINK,
                    f"{HeaderSource.CAPACITY}_x": HeaderSource.CAPACITY,
                },
                inplace=True,
            )
    ...
    def get_html(self, url: str) -> BeautifulSoup | None:
        ...
        try:
            html = requests.get(
                ...
                verify=False,
                auth=HTTPBasicAuth(
                    self.config.scan_username, self.config.scan_password
                ),
            )
        ...
        try:
            ...
            if os.path.exists(source_path):
                ...
                if not df_old.empty:
                    ...
                    df_old.sort_values(
                        by=[HeaderSource.MODEL, HeaderSource.NAME], inplace=True
                    )
            ...
            if layer.upper() == LayerName.BORDE or layer.upper() == LayerName.BRAS:
                ...
                data[HeaderSource.CAPACITY] = data[HeaderSource.CAPACITY].astype(float)
            data.sort_values(
                by=[HeaderSource.MODEL, HeaderSource.CAPACITY, HeaderSource.NAME],
                inplace=True,
            )
