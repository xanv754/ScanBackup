...
class BordeUpdaterHandler(UpdaterHandler):
    ...
    def get_data(self, folderpath: str | None = None, date: str | None = None, force: bool = False) -> pd.DataFrame:
        try:
            ...
            log.error(f"Failed to data load of Borde layer. {e}")
    ...
    def load_data(self, data: pd.DataFrame, uri: str | None = None) -> bool:
        try:
            if data.empty: 
                log.warning("The system received empty data Borde when it updated.")
                return False
            ...
            try:
                ...
                log.error(f"Failed to validate data with the model. Borde updater system has suspended. {e}")
                return False
            else:
    ...
    def _get_info_interfaces(self, soup: BeautifulSoup, model: str) -> list[Source]:
        ...
        try:
            ...
            for interface in interfaces:
                ...
                preffix = name.split("-")[0].strip().split(junk_word)[1].strip()
