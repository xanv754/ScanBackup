...
class UpdaterHandler:
    ...
    def __init__(self, layer: str, uri: str, date: str | None = None, force: bool = False) -> None:
        if layer == LayerName.DAILY_REPORT:
            self._updater_daily_summary(uri=uri, date=date, force=force)
        else: self._updater_bbip(layer=layer, uri=uri, date=date, force=force)
    
    def _updater_bbip(self, layer: str, uri: str, date: str | None, force: bool) -> None:
        ...
        try:
            ...
            if status_operation: log.info(f"Data de {layer} cargado exitosamente")
            else: log.error(f"Carga de la data de {layer} fallida")
    ...
    def _updater_daily_summary(self, uri: str, date: str | None, force: bool) -> None:
        ...
        try:
            ...
            if status_operation: log.info("Actualización de reportes diarios cargado exitosamente")
            else: log.error(f"Carga de los reportes diarios fallido")  
        except Exception as e:
