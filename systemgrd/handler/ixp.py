...
from systemgrd.database import BBIPQuery, IXPMongoQuery, DailyReportQuery, DailyReportMongoQuery
from systemgrd.handler.scan import ScanHandler
from systemgrd.utils import Validate, log


class IXPHandler(ScanHandler):
    """Class to get data of IXP layer."""
    ...
    def __init__(self, uri: str | None = None):
        try:
            self.ixp_query = IXPMongoQuery(uri=uri)
            self.daily_query = DailyReportMongoQuery(uri=uri)
        except Exception as e:
            log.error(f"IXP handler. Failed connecting to the database. {e}")
    ...
    def get_all_interfaces(self):
        try:
            ...
            df_interfaces = self.ixp_query.get_interfaces()
        except Exception as e:
            log.error(f"IXP handler. Failed to get all interfaces of IXP layer. {e}")
    ...
    def get_all_interfaces_by_date(self, date: str):
        try:
            ...
            df_interfaces = self.ixp_query.get_interfaces_by_date(date=date)
        except Exception as e:
            log.error(f"IXP handler. Failed to get all interfaces of IXP layer. {e}")
            return pd.DataFrame(columns=header_bbip)
        else:
    ...
    def get_all_daily_report(self, date: str | None = None):
        try:
            ...
            df_daily_report = self.daily_query.get_report(layer_type=LayerName.IXP, date=date)
        except Exception as e:
            log.error(f"IXP handler. Failed to get all daily report of IXP layer. {e}")
