...
class GenerateDailyReport:
    ...
    def _calculate_summary_data(self, interface: str, type: str, capacity: float, data: pd.DataFrame) -> pd.DataFrame:
        try:
            in_average = (data[HeaderBBIP.IN_VALUE].mean()) * FACTOR_BBIP
            out_average = (data[HeaderBBIP.OUT_VALUE].mean()) * FACTOR_BBIP
            ...
            summary_data = pd.DataFrame({
                ...
                HeaderDailyReport.DATE: [self.date],
                HeaderDailyReport.CAPACITY: [capacity],
                HeaderDailyReport.IN_PROM: [in_average],
                HeaderDailyReport.IN_MAX: [in_max_average],
                HeaderDailyReport.OUT_PROM: [out_average],
                HeaderDailyReport.OUT_MAX: [out_max_average],
                HeaderDailyReport.USE: [use]
            })
            return summary_data
        except:
            log.error(f"Fallo al generar el resumen diario de la interfaz: {interface}")
        ...
        try:
            for layer in self.layers:
                ...
                if not old_daily_report.empty and not summary_report.empty:
                    ...
                    summary_report.reset_index(drop=True, inplace=True)
                summary_report.to_csv(self._get_daily_path(layer), index=False, sep=self._separator, decimal=self._export_decimal)
...
def generate_daily_summary(layer: str | None = None, date: str | None = None) -> None:
    if date: report = GenerateDailyReport(date=date, layer=layer)
    else: report = GenerateDailyReport(layer=layer)
    report.run()
