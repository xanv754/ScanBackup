...
class Query(unittest.TestCase):
    ...
    def test_get(self):
        ...
        response = database.get_report(
            layer_type=example_report.typeLayer, date=example_report.date
        )
        print(response)
        self.mongo_db_test.clean()
        self.assertFalse(response.empty)
        self.assertEqual(
            response[DailyReportFieldName.DATE].iloc[0], example_report.date
        )
        self.assertEqual(
            response[DailyReportFieldName.TYPE_LAYER].iloc[0], example_report.typeLayer
        )
