...
class Scrapping(ABC):
    ...
    def _compare_sources_old(
        self, new_data: pd.DataFrame, old_data: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        try:
            ...
            log.error(
                f"Fallo al comparar las fuentes existentes con las obtenidas recientemente - {error}"
            )
    ...
    def get_html(self, url: str) -> BeautifulSoup | None:
        ...
        try:
            if self._dev:
                env = UserEnvironment(dev=True)
            else:
                env = UserEnvironment()
            username = env.get_username()
            password = env.get_password()
            html = requests.get(
                url, verify=False, auth=HTTPBasicAuth(username, password)
            )
            soup = BeautifulSoup(html.text, "html.parser")
            if not soup:
                raise Exception("No se obtuvo información del HTML")
        except Exception as error:
            log.error(
                f"Fallo al obtener la estructura HTML de la página: {self._url}, capa: {self._layer} - {error}"
            )
            return None
        else:
            return soup
        ...
        try:
            ...
            if os.path.exists(source_path):
                existing_sources = pd.read_csv(
                    source_path, sep=self._separator, header=None, names=header_source
                )
                if not self._add_sources:
                    self._create_backup(existing_sources)
                new_sources = self._compare_sources_old(
                    new_data=new_sources, old_data=existing_sources
                )
                if new_sources.empty:
                    raise Exception("La comparación de fuentes falló")
            new_sources.sort_values(
                [HeaderSource.MODEL, HeaderSource.CAPACITY, HeaderSource.NAME],
                inplace=True,
            )
            new_sources.to_csv(
                source_path, sep=self._separator, header=False, index=False
            )
        except Exception as error:
            log.error(
                f"Fallo al guardar las fuentes nuevas de la capa {self._layer} - {error}"
            )
            return False
        else:
            return True
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if not sources:
                raise Exception(
                    f"No se obtuvo ninguna de la fuentes de la capa {self._layer}"
                )
            return self.save_sources(sources)
        except Exception as error:
            log.error(
                f"Falló al realizar la actualización de fuentes de la capa {self._layer} - {error}"
            )
            return False
